...
var saleSwitchAjax = function(appId,obj){
	$.ajax({
		type:"get",
		url:"sale.json",
		data:{appId:appId,saleSwitch:obj.attr("saleSwitch")},
		dataType:"json",
		success:function(data){
			if(data.errorCode === '0'){
				if(data.resultMsg === ".+"){//操作成功
					if("open" === obj.attr("saleSwitch")){
						...
						obj.html("下架");
						obj.parent().next().next().children(".modifyVersion").attr("statusname","已上架");
						obj.parent().next().next().next().children(".modifyAppInfo").attr("statusname","已上架");
						...
						obj.html("上架");
						obj.parent().next().next().children(".modifyVersion").attr("statusname","已下架");
						obj.parent().next().next().next().children(".modifyAppInfo").attr("statusname","已下架");
