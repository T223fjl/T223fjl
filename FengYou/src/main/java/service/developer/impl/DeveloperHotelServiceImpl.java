...
import dao.developer.DeveloperHotelDao;
import pojo.Hotel;
import service.developer.DeveloperHotelService;
import util.PinYinUtil;

@Service
public class DeveloperHotelServiceImpl implements DeveloperHotelService {

	@Autowired
	private DeveloperHotelDao hotelDao;

	/***
	 * 多条件查询酒店信息
	 */
	public List<Hotel> queryHotel(@Param("hotelRating") int hotelRating, @Param("level1") int level1,
			@Param("level2") int level2, @Param("level3") int level3, @Param("bigPrice") int bigPrice,
			@Param("smallPrice") int smallPrice, @Param("sort") String sort, @Param("desc") String desc) {
		List<Hotel> queryHotel = hotelDao.queryHotel(hotelRating, level1, level2, level3, bigPrice, smallPrice, sort,
				desc);
	...
	public PageInfo<Hotel> findHotelList(@Param("hotelRating") int hotelRating, @Param("level1") int level1,
			@Param("level2") int level2, @Param("level3") int level3, @Param("bigPrice") int bigPrice,
			@Param("smallPrice") int smallPrice, @Param("sort") String sort, @Param("desc") String desc, int page,
			int size) {
