	...
	<div class="mt-20">
	 ...
	 <th width="40"><input name="checks" type="checkbox"  ></th>
        ...
        <th width="100">操作</th>
			...
			<tbody>
		<c:forEach var="order" items="${orders}" step="1" varStatus="statu">
		<tr class="text-c" id="${order.id}">
			<td><input type="checkbox" value="1" name=""></td>
			 <td>${order.id}</td>
			 
			     <td><a style="cursor:pointer" class="text-primary"  href="userview?id=${user.id}">${ order.orderNo}</a></td>
			      <td>${ order.hotelnames}</td>
   <td class="text-l"><fmt:formatDate value="${ order.checkInDate}" type="both"/></td>
        <td><fmt:formatDate value="${ order.checkOutDate}" type="both"/></td>
        
        
          <c:if test="${ order.orderStatus==0}">
        <td class="user-status"><span class="label label-success">待支付</span>
        
        </td>
        </c:if>
        
        <c:if test="${ order.orderStatus==1}">
        <td class="user-status"><span class="label label-success">已支付</span>
        
        </td>
        </c:if>
        
            <c:if test="${ order.orderStatus==2}">
        <td class="user-status"><span class="label label-success">已超时</span>
        
        </td>
        </c:if>
        
            <c:if test="${ order.orderStatus==3}">
        <td class="user-status"><span class="label label-success">已取消</span>
        
        </td>
        </c:if>
        
        <td class="f-14 td-manage"> <a style="text-decoration:none" class="ml-5"  onclick="del(${order.id})" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a></td>
				</tr>
				 </c:forEach>
			</tbody>
		</table>
