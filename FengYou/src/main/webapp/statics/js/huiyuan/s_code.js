...
if (document.location.host.toLowerCase().indexOf("elong") > 0 || document.location.host.toLowerCase().indexOf("hzypms") > 0) {
	...
	function s_doPlugins(s) {
		...
		if (s.channel == "hotels") {
			...
			} else if (s.getElongOPageNameVal().toLowerCase() == "hoteldetail" || s.getElongOPageNameVal().toLowerCase() == "hoteldetailreviews" || s.getElongOPageNameVal().toLowerCase() == "hotelphotos" || s.getElongOPageNameVal().toLowerCase() == "hoteldetailmap") {
				s.events = "event8," + s.events; //Shopper
				s.prop4 = "actual hotel shopper";
			...
			} else if (typeof(trackingOBJ) != "undefined" && trackingOBJ.oPageName != "" && trackingOBJ.oPageName.toLowerCase().indexOf("opaquelist") >= 0) {
				s.events = "event8," + s.events; //Shopper
	...
	*/
	...
	s.getQueryParam = new Function("qp", "d", "" + "var s=this,v='',d=d?d:'',i,t;while(qp){i=qp.indexOf(',');i=i<0?qp.l" + "ength:i;t=s.gcgi(qp.substring(0,i));if(t)v+=v?d+t:t;qp=qp.substring" + "(i==qp.length?i:i+1)}return v");
	...
	s.vpr = new Function("vs", "v",
		"var s=this,k=vs.substring(0,2)=='s_'?vs.substring(2):vs;s['vpv_'+k]=" + "v;s['vpm_'+k]=1");
	s.dt = new Function("tz", "t",
		"var d=new Date;if(t)d.setTime(t);d=new Date(d.getTime()+(d.getTimezo" + "neOffset()*60*1000));return new Date(Math.floor(d.getTime()+(tz*60*" + "60*1000)))");
	s.vh_gt = new Function("k", "v",
		"var s=this,vh='|'+s.c_r('s_vh_'+k),vi=vh.indexOf('|'+v+'='),ti=vi<0?" + "vi:vi+2+v.length,pi=vh.indexOf('|',ti),t=ti<0?'':vh.substring(ti,pi" + "<0?vh.length:pi);return t");
	s.vh_gl = new Function("k",
		"var s=this,vh=s.c_r('s_vh_'+k),e=vh?vh.indexOf('='):0;return vh?(vh." + "substring(0,e?e:vh.length)):''");
	s.vh_s = new Function("k", "v",
		"if(k&&v){var s=this,e=new Date,st=e.getTime(),y=e.getYear(),c='s_vh_" + "'+k,vh='|'+s.c_r(c)+'|',t=s.vh_gt(k,v);e.setYear((y<1900?y+1900:y)+" + "5);if(t)vh=s.rep(vh,'|'+v+'='+t+'|','|');if(vh.substring(0,1)=='|')" + "vh=vh.substring(1);if(vh.substring(vh.length-1,vh.length)=='|')vh=v" + "h.substring(0,vh.length-1);vh=v+'=[PCC]'+(vh?'|'+vh:'');s.c_w(c,vh," + "e);if(s.vh_gt(k,v)!='[PCC]')return 0;vh=s.rep(vh,'[PCC]',st);s.c_w(" + "c,vh,e)}return 1");
