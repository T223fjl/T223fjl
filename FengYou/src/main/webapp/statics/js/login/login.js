...
require(['jquery', 'web', 'dialog_new', 'plugins/validatorClass', 'fingerprint.pc'], function($, E, D, validatorClass, Finger) {
		...
		showHelp: function(event) {
			...
			function showMidAnimate() {
				if (element.parent().prev().is(":animated")) {} else {
					element.parent().prev().fadeIn(35, function() {}).animate({
						left: "85px"
					}, 70, function() {
						showLastAnimate();
					});
			...
			function closeLastAnimate() {
				if (element.parent().prev().is(":animated")) {} else {
					element.parent().prev().animate({
						...
						element.parent().prev().fadeOut(35, function() {});
					});
				}
			}
	...
	function loginTc(data) {
		...
		var param = {
			...
			ELGuid: data.sessionGuid,
			ELEsid: data.esid
		};
		var loginTcUrl = '//m.t.ly.com/passport/login/SetELongCookie?' + jQuery.param(param);
		iframe.attr("src", loginTcUrl);
