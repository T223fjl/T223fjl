...
function deleteBill(obj){
	$.get(path+"/DeleteBill","billid="+obj.attr("billid"),back);
	function back(datas) {
		var data=eval("("+datas+")");
...
$(function(){
	$(".viewBill").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/QueryBill?method=view&billid="+ obj.attr("billid");
	});
	
	$(".modifyBill").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/UpdateBill?billid="+ obj.attr("billid");
	});
	$('#no').click(function () {
		cancleBtn();
	});
